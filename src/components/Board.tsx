...
export const BoardComponent: FC<{
  ...
  const opponentState = useOpponentState((state) => state)

  const winnerInfo = (() => {
    if (playerState.playerColor === oppositeColor(turn)) {
      const { uuid: loserUuid } = opponentState
      return {
    ...
    return { uuid: null, room: null, loserUuid: null }
  })()
  ...
  useEffect(() => {
    ...
    if (gameOverType) {
      setGameOver({
        ...
        winnerUuid: winnerInfo.uuid,
        loserUuid: winnerInfo.loserUuid,
        gameSessionUuid: winnerInfo.room,
      })
    }
  }, [board, turn])
