...
export type GameOverType = `checkmate` | `dropped` | `stalemate`
...
//if player don't have any pieces left, the game is over(check players pieces)
export const detectNoPiecesGameOver = (
  board: Board,
  turn: Color
): GameOverType | null => {
  // Check if any piece of the current player's color exists on the board
  const hasPieces = board.flat().some((tile) => tile.piece?.color === turn);

  // If no pieces remain, the game is over
  return hasPieces ? null : `checkmate`;
}
